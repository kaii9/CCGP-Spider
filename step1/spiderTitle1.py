...
def writer_excel(data, directory, filename, head):
    ...
    """
    file_path = os.path.join(directory, filename + 'new.csv')
        ...
        for type in range(9,11):
            for year in [2024,2023]:
                ...
                for month in range(12, 0, -1):
                    start_time = f"{year}:01:01" if month == 1 else f"{year}:{month:02d}:02"
                    end_time = ""
                    if month == 12:
                        if y == 0:
                            end_time = f"{curr_year}:12:14"
                        else:
                    ...
                    while flag!=True:
                        ...
                        if Maxpage == -1 and flag==False:
                            end_time1 = f"{year}:{month:02d}:15"
                            start_time1 = f"{year}:{month:02d}:16"
                            while flag!=True:
                                if page > 1:
                                    time.sleep(60)
                                    sheetdata = []
                                ...
                                writer_excel(sheetdata ,output_dir,str(type),head)
                            flag = False
                            page=1
                            while flag!=True:
                                if page > 1:
